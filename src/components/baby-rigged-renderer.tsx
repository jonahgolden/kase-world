...
export function BabyRiggedView({ entity }: { entity: Entity }) {
	...
	useEffect(() => {
		...
		// action.timeScale = 1.0; // Normal speed to see if that helps
		// action.loop = THREE.LoopRepeat; // Loop mode
		// action.clampWhenFinished = false; // Don't clamp at the end
		// action.paused = false; // Make sure it's not paused
...
export function BabyRiggedRenderer() {
	const baby = useQueryFirst(IsBaby, Transform);
	return baby ? <BabyRiggedView entity={baby} /> : null;
}
