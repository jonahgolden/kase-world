...
export function ev(s: State, e: GameEvent) {
  s.events.push(e)
}
...
export function coastClear(s: State, x: number, z: number): number {
  if (!pointInRing(x, z, s.arena.ring)) return -1
  return closestOnRing(x, z, s.arena.ring).d
}

export function featureAt(s: State, x: number, z: number, kinds: Feature['kind'][], pad = 0): Feature | null {
...
export function randomInside(s: State, margin: number, tries = 40): { x: number; z: number } | null {
  const A = s.arena
...
export function farPoint(s: State, fromX: number, fromZ: number, margin: number, samples = 16, ok?: (x: number, z: number) => boolean): { x: number; z: number } {
  let best = { x: 0, z: 0 }
  let bestD = -1
...
export function addTimeBonus(s: State, seconds: number, x: number, z: number, label: string) {
...
export function inCone(px: number, pz: number, facing: number, tx: number, tz: number, tr: number, rangeLen: number, halfAngle: number) {
  const dx = tx - px
  const dz = tz - pz
  const d = Math.hypot(dx, dz)
...
export function spawnPickup(s: State, kind: PickupKind, x: number, z: number, vy = 0) {
...
export function damageProp(s: State, pr: Prop, dmg: number, wreckScale = 1, src: DamageSource = 'other') {
  if (pr.broken) return
...
export function moveToward(o: { x: number; z: number; vx: number; vz: number; facing: number }, tx: number, tz: number, speed: number, k = 8) {
  const dx = tx - o.x
  const dz = tz - o.z
  const d = Math.hypot(dx, dz)
  if (d < 0.05) return d
