// Keyboard + touch -> Input. The only place that knows about DOM events.
...
export class InputDriver {
  ...
  read(): Input {
    ...
    return {
      mx,
      mz,
      scream: this.held.scream || this.key('Space', 'KeyJ'),
      poop: this.held.poop || this.key('KeyE', 'KeyK', 'Enter'),
      jump: this.held.jump || this.key('ShiftLeft', 'ShiftRight', 'KeyL', 'KeyQ'),
    }
  }

  anyPressed(): boolean {
    return this.keys.size > 0 || this.held.scream || this.held.poop || this.held.jump || this.joy.active
  }
}
